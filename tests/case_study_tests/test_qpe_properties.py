...
class TestQPEProperties(unittest.TestCase):
    ...
    def test_eigenvector_property(self):
        # run the test
        runner = TestRunner([LowerRegisterUnchangedByEigenvector], self.num_inputs, 101, 5000)
        runner.run_tests()
    ...
    def test_sum_eigenvector_property(self):
        # run the test
        runner = TestRunner([PhaseEstimationSumEigenvectors], self.num_inputs, 102, 5000)
        runner.run_tests()
        # the property should pass
        assert runner.list_failing_properties() == []
    ...
    def test_sum_eigenvector_different_eigenvalues_property(self):
        # run the test
        runner = TestRunner([PhaseEstimationSumDifferentEigenvectors], self.num_inputs, 102, 5000)
        runner.run_tests()
        # the property should pass
        assert runner.list_failing_properties() == []
    ...
    def test_phase_correctly_estimated_with_enough_qubits(self):
        # run the test
        runner = TestRunner([PhaseCorrectlyEstimatedEnoughQubits], self.num_inputs, 106, 5000)
        runner.run_tests()
        # the property should pass
        assert runner.list_failing_properties() == []
