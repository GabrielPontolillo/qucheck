...
from QiskitPBT.case_studies.quantum_teleportation.unitary_before_teleport_equal_unitary_after_teleport_property import UnitaryBeforeAndAfterTeleport
...
class TestQTProperties(unittest.TestCase):
    ...
    def test_equal_input_output(self):
        # run the test
        runner = TestRunner([Inq0EqualOutq2], self.num_inputs, 1)
        runner.run_tests()
    ...
    def test_not_teleported_plus(self):
        # run the test
        runner = TestRunner([NotTeleportedPlus], self.num_inputs, 1)
        runner.run_tests()
        # the property should pass
        assert runner.list_failing_properties() == []
    ...
    def test_unitary_before_and_after_teleport(self):
        # run the test
        runner = TestRunner([UnitaryBeforeAndAfterTeleport], self.num_inputs, 1)
        runner.run_tests()
        # the property should pass
        assert runner.list_failing_properties() == []
