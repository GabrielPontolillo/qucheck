...
class TestTestRunner(TestCase):
    ...
    def test_run_tests(self):
        ...
        test_runner = TestRunner([Inq0EqualOutq2], num_inputs,  548, measurements)
        # run the tests
        ...
        self.assertEqual(data.number_circuits_executed, num_inputs*2*3)
    ...
    def test_run_tests_same_number_circuits_executed_when_using_same_property_twice(self): # fails, should be automatically fixed when optimisation is fixed
        ...
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([Inq0EqualOutq2, Inq0EqualOutq2], num_inputs,  548, measurements)
        # run the tests
        data = test_runner.run_tests()
        self.assertEqual(test_runner.list_failing_properties(), [])
        ...
        test_runner2 = TestRunner([Inq0EqualOutq2], num_inputs,  548, measurements)
    ...
    def test_run_tests_different_properties_same_input_optimisation_check(self): # fails, should be automatically fixed when optimisation is fixed
        ...
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([Inq0EqualOutq2, NotTeleportedPlus], num_inputs,  548, measurements)
        # run the tests
        data = test_runner.run_tests()
        self.assertEqual(test_runner.list_failing_properties(), [])
        self.assertEqual(test_runner.list_passing_properties(), [Inq0EqualOutq2, NotTeleportedPlus])
        self.assertEqual(data.number_circuits_executed, (num_inputs * 2 * 3) + 3)
        ...
        test_runner3 = TestRunner([NotTeleportedPlus], num_inputs, 548, measurements)
        ...
        self.assertEqual(data.number_circuits_executed, (num_inputs * 1 * 3) + 3)


    def test_same_seeds(self):
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([Inq0EqualOutq2, Inq0EqualOutq2], 3, 1, measurements)
        # run the tests
        test_runner.run_tests()
        save_seeds = test_runner.seeds_list_dict
        # create an instance of the test runner
        test_runner2 = TestRunner([Inq0EqualOutq2, Inq0EqualOutq2], 3, 1, measurements)
        # run the tests
    ...
    def test_different_seeds(self):
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([Inq0EqualOutq2, IdentityProperty], 3, 1, measurements)
        # run the tests
        test_runner.run_tests()
        save_seeds = test_runner.seeds_list_dict
        ...
        test_runner2 = TestRunner([Inq0EqualOutq2, IdentityProperty], 3, 2, measurements)
        # run the tests
        test_runner2.run_tests()
        save_seeds2 = test_runner2.seeds_list_dict
    ...
    def test_failing_precondition(self):
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([FailingPrecondition], 2,  548, measurements)
        # run the tests
        test_runner.run_tests()
    ...
    def test_repeated_property_does_not_run_more_circuits(self):
        measurements = 5000
        # create an instance of the test runner
        test_runner = TestRunner([Inq0EqualOutq2], 3, 1, measurements)
        # run the tests
