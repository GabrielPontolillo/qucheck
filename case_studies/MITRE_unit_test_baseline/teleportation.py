...
class TeleportationTests():
    ...
    def run_test(self, description, iterations, prep_function):
        ...
        for entanglement_state in range(0, 4):
            ...
            result = backend.run(transpiled_circuit, shots=iterations).result()
    ...
    def test_zero(self, iterations):
        """
        Tests teleportation on the |0> state.
        """

        return self.run_test("Teleport |0>", iterations, self.prepare_zero_state)


    def test_one(self, iterations):
        """
        Tests teleportation on the |1> state.
        """

        return self.run_test("Teleport |1>", iterations, self.prepare_one_state)


    def test_plus(self, iterations):
        """
        Tests teleportation on the |+> state.
        """

        return self.run_test("Teleport |+>", iterations, self.prepare_plus_state)


    def test_minus(self, iterations):
        """
        Tests teleportation on the |-> state.
        """

        return self.run_test("Teleport |->", iterations, self.prepare_minus_state)


    def test_i_plus(self, iterations):
        """
        Tests teleportation on the |i+> state.
        """

        return self.run_test("Teleport |i+>", iterations, self.prepare_i_plus_state)


    def test_i_minus(self, iterations):
        """
        Tests teleportation on the |i-> state.
        """

        return self.run_test("Teleport |i->", iterations, self.prepare_i_minus_state)


    def test_weird_rotation(self, iterations):
        """
        Tests teleportation on the uneven superposition.
        """

        return self.run_test("Teleport weird rotation", iterations, self.prepare_weird_rotation)
...
if __name__ == '__main__':
    ...
    def run_tests(circuit_function, path, shots):
        with patch("__main__.quantum_teleportation", circuit_function):
            reload_classes(path)
            tests = [
                TeleportationTests().test_minus(shots),
                TeleportationTests().test_one(shots),
                TeleportationTests().test_plus(shots),
                TeleportationTests().test_zero(shots),
                TeleportationTests().test_i_minus(shots),
                TeleportationTests().test_i_plus(shots),
                TeleportationTests().test_weird_rotation(shots),
            ]
            return all(tests)
