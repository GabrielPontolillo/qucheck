...
def run_grover_search(number_of_qubits, oracle, oracle_args, grovers_algorithm):
    """
    ...
    oracle_circuit = QuantumCircuit(qubits, work)

    if oracle_args is None:
        oracle(oracle_circuit, qubits, work)
    else:
        oracle(oracle_circuit, qubits, work, oracle_args)

    # print(oracle_circuit)
    ancillas_size = oracle_circuit.qregs[2].size
    iterations = round(2 ** (len(qubits) / 2))
    circuit = grovers_algorithm(oracle_circuit, iterations, ancillas=ancillas_size)
