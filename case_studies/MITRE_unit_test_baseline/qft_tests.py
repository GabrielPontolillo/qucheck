...
class QftTests():
    ...
    def run_iqft_with_waveform_samples(self, number_of_qubits, sample_rate,
        ...
        qft = quantum_fourier_transform(number_of_qubits).inverse()
    ...
    def test_1hz_sine_8_samples(self):
        ...
        """
        print("Running 1 Hz sine test with 8 samples at 8 samples per second.")
        return self.run_iqft_with_waveform_samples(3, 8, 1, self.prepare_1hz_sine_8_samples, None)

    def test_1hz_cosine_8_samples(self):
        """
        Tests QFT by giving it a cosine wave of 1 Hz, with 8 samples, at 8 samples per second.
        """
        print("Running 1 Hz cosine test with 8 samples at 8 samples per second.")
        return self.run_iqft_with_waveform_samples(3, 8, 1, self.prepare_1hz_cosine_8_samples, None)

    def test_2hz_sine_8_samples(self):
        """
        Tests QFT by giving it a sine wave of 2 Hz, with 8 samples, at 8 samples per second.
        """
        print("Running 2 Hz sine test with 8 samples at 8 samples per second.")
        return self.run_iqft_with_waveform_samples(3, 8, 2, self.prepare_2hz_sine_8_samples, None)

    def test_2hz_cosine_8_samples(self):
        """
        Tests QFT by giving it a cosine wave of 2 Hz, with 8 samples, at 8 samples per second.
        """
        print("Running 2 Hz cosine test with 8 samples at 8 samples per second.")
        return self.run_iqft_with_waveform_samples(3, 8, 2, self.prepare_2hz_cosine_8_samples, None)
    ...
    def test_period_6(self):
        ...
        qft = quantum_fourier_transform(input_length).inverse()
        circuit.compose(qft, inplace=True)
...
if __name__ == '__main__':
    ...
    def run_tests(circuit_function, path, shots):
        with patch("__main__.quantum_fourier_transform", circuit_function):
            reload_classes(path)
            tests = [
                QftTests().test_period_6(),
                QftTests().test_sine(),
                QftTests().test_1hz_sine_8_samples(),
                QftTests().test_1hz_cosine_8_samples(),
                QftTests().test_2hz_sine_8_samples(),
                QftTests().test_2hz_cosine_8_samples()
            ]
            return all(tests)
    ...
    # mutants = [(f"em", i) for i in range(1)]
    shots_list = [7500]
